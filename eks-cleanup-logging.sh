...
echo "Starting cleanup in $REGION for cluster: $CLUSTER_NAME" | tee -a $LOGFILE
...
delete_or_preview() {
  ...
  if [ -n "$items" ]; then
    echo "$resource_type found: $items" | tee -a $LOGFILE
    for item in $items; do
      if [ "$DRYRUN" = true ]; then
        echo "Would delete $resource_type: $item" | tee -a $LOGFILE
        ...
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
          ...
          echo "Deleted $resource_type: $item" | tee -a $LOGFILE
          DELETED=$((DELETED+1))
        else
          echo "Skipped $resource_type: $item" | tee -a $LOGFILE
          SKIPPED=$((SKIPPED+1))
        fi
      fi
    done
  else
    echo "No $resource_type found." | tee -a $LOGFILE
  fi
}
...
echo "Would delete (dry-run): $WOULD_DELETE" | tee -a $LOGFILE
echo "Deleted: $DELETED" | tee -a $LOGFILE
echo "Skipped: $SKIPPED" | tee -a $LOGFILE
echo "-----------------------------------" | tee -a $LOGFILE
echo "Cleanup check complete ✅" | tee -a $LOGFILE
